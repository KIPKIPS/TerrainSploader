...
public class TerrainSliceEditor : Editor {
    ...
    private static void Slicing() {
        ...
        if (Directory.Exists(savepath)) Directory.Delete(savepath, true); //存在文件夹则删除文件夹及其内部文件
        ...
        //得到新地图分辨率
        int newAlphaMapResolution = terrainData.alphamapResolution / TerrainConfig.Slice;
        ...
        int terrainsWide = TerrainConfig.Slice;
        int terrainsLong = TerrainConfig.Slice;
        int newDetailResolution = terrainData.detailResolution / TerrainConfig.Slice;
        ...
        try {
            ...
            for (int x = 0; x < terrainsWide; ++x) {
                for (int y = 0; y < terrainsLong; ++y) {
                    ...
                    string terrainName = terrain.name + TerrainConfig.SplitChar + y + TerrainConfig.SplitChar + x + ".asset";
                    ...
                    //设置分辨率参数
                    newData.heightmapResolution = (terrainData.heightmapResolution - 1) / TerrainConfig.Slice;
                    newData.alphamapResolution = terrainData.alphamapResolution / TerrainConfig.Slice;
                    newData.baseMapResolution = terrainData.baseMapResolution / TerrainConfig.Slice;

                    //设置大小
                    newData.size = new Vector3(oldSize.x / terrainsWide, oldSize.y, oldSize.z / terrainsLong);

                    //设置地形原型
                    SplatPrototype[] newSplats = new SplatPrototype[splatPrototypes.Length];
                    for (int i = 0; i < splatPrototypes.Length; ++i) {
                        newSplats[i] = new SplatPrototype();
                        newSplats[i].texture = splatPrototypes[i].texture;
                    ...
                    //设置混合贴图
                    float[,,] alphaMap = new float[newAlphaMapResolution, newAlphaMapResolution, splatPrototypes.Length];
                    ...
                    int[] layers = terrainData.GetSupportedLayers(x * newData.detailWidth - 1,
            ...
            for (int i = 0; i < treeInst.Length; i++) {
                Vector3 origPos = Vector3.Scale(new Vector3(oldSize.x, 1, oldSize.z),
                ...
                Vector3 tempVect = new Vector3((origPos.x - newWidth * column) / newWidth, origPos.y,
    ...
    private static void WriteTerrainInfo(Terrain terrain, string path) {
        ...
        int size = (int)terrain.terrainData.size.x / TerrainConfig.Slice;
